document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        /* --- SUSTAINABILITY VIEW CHARTS --- */

        // ISI Gauge (half-circle doughnut)
        if (data.sustainability && document.getElementById('isiGauge')) {
            const isi = data.sustainability.isi || 0;
            const isiColor = isi < 0.70 ? '#22C55E' : isi <= 0.85 ? '#EAB308' : '#EF4444';

            const ctxISI = document.getElementById('isiGauge').getContext('2d');
            new Chart(ctxISI, {
                type: 'doughnut',
                data: {
                    datasets: [{
                        data: [isi, Math.max(0, 1 - isi)],
                        backgroundColor: [isiColor, 'rgba(255,255,255,0.05)'],
                        borderWidth: 0
                    }]
                },
                options: {
            ...
            document.getElementById('isi-value').innerText = isi.toFixed(2);
            document.getElementById('isi-status').innerText = data.sustainability.isi_status || 'Normal';
        }

        // CO2 and PCES values
        if (data.sustainability) {
            document.getElementById('co2-tonnes').innerText = (data.sustainability.co2_tonnes_annual / 1000000).toFixed(1) + 'M';
            document.getElementById('pces-score').innerText = data.sustainability.pces_score.toFixed(1);
            document.getElementById('pces-rating').innerText = data.sustainability.pces_rating;
        }

        // SHAP Feature Importance
        if (data.shap_features && data.shap_features.length > 0 && document.getElementById('shapChart')) {
            const ctxShap = document.getElementById('shapChart').getContext('2d');
            new Chart(ctxShap, {
                type: 'barh',
                data: {
                    labels: data.shap_features.map(f => f.feature),
                    datasets: [{
                        label: 'Feature Importance (SHAP)',
                        data: data.shap_features.map(f => f.importance),
        ...
        // What-If Simulator Event Listeners
        const fuelMult = document.getElementById('fuel-mult');
        const gdpDelta = document.getElementById('gdp-delta');
        const safPct = document.getElementById('saf-pct');
        const simulateBtn = document.getElementById('simulate-btn');

        if (fuelMult) {
            fuelMult.addEventListener('input', (e) => {
                document.getElementById('fuel-mult-val').innerText = parseFloat(e.target.value).toFixed(1);
            });
        }

        if (gdpDelta) {
            gdpDelta.addEventListener('input', (e) => {
                document.getElementById('gdp-delta-val').innerText = parseFloat(e.target.value).toFixed(1);
            });
        }

        if (safPct) {
            safPct.addEventListener('input', (e) => {
                document.getElementById('saf-pct-val').innerText = e.target.value;
            });
        }
        ...
        if (simulateBtn) {
            simulateBtn.addEventListener('click', async () => {
                const payload = {
                    fuel_multiplier: parseFloat(fuelMult.value),
                    gdp_delta: parseFloat(gdpDelta.value),
                    saf_pct: parseFloat(safPct.value) / 100
                };
                ...
                try {
                    ...
                    if (result.recommendations && result.recommendations.length > 0) {
                        document.getElementById('simulate-text').innerText =
                            `Scenario impact: ${result.recommendations[0].action}`;
